import shutil # Keep for potential future use, though _clean_up_output_dirs is removed
...
from wpp.config import get_wpp_report_dir, get_wpp_log_dir # Import getters for dynamic paths
...
def test_regression(setup_wpp_root_dir, run_decrypt_script) -> None:
    ...
    for generated_report in generated_reports:
        ...
        assert ref_file_to_compare is not None, \
            f"No matching reference report found for generated report: {generated_report.name}."
    ...
    for generated_log in generated_logs:
        ...
        assert ref_log_to_compare is not None, \
            f"No matching reference log found for generated log: {generated_log.name}."
