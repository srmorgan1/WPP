    ...
    def match(self, description: str, db_cursor: Optional[sqlite3.Cursor]) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if match:
            ...
            if db_cursor:
                ...
                (property_ref in ["093", "094", "095", "096", "099", "124", "132", "133", "134"]) or
                (property_ref in ["020", "022", "039", "053", "064"] and match.group(3)[-1] != "Z")
            ):
                return None, None, None
            return property_ref, block_ref, tenant_ref
        return None, None, None
...
class NoHyphenRegexStrategy(MatchingStrategy):
    def match(self, description: str, db_cursor: Optional[sqlite3.Cursor]) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        match = (
            re.search(PBT_REGEX_NO_HYPHENS, description) or
            re.search(PBT_REGEX_NO_HYPHENS_SPECIAL_CASES, description) or
            re.search(PBT_REGEX_NO_TERMINATING_SPACE, description) or
            re.search(PBT_REGEX_NO_BEGINNING_SPACE, description)
        )
        if match:
            property_ref, block_ref, tenant_ref = getPropertyBlockAndTenantRefsFromRegexMatch(match)
            if db_cursor and not doubleCheckTenantRef(db_cursor, tenant_ref, description):
...
def getPropertyBlockAndTenantRefs(reference: str, db_cursor: Optional[sqlite3.Cursor] = None) -> Tuple[Optional[str], Optional[str], Optional[str]]:
