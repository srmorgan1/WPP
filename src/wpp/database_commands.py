...
class InsertChargeCommand(DatabaseCommand):
    ...
    def execute(self, cursor: sqlite3.Cursor, logger: logging.Logger) -> None:
        cursor.execute(self.insert_sql, (
            self.charge_data.fund_id,
            self.charge_data.category_id, 
            self.charge_data.type_id,
            self.charge_data.at_date,
            self.charge_data.amount,
            self.charge_data.block_id
        ))
...
class InsertTransactionCommand(DatabaseCommand):
    ...
    def execute(self, cursor: sqlite3.Cursor, logger: logging.Logger) -> None:
        ...
        logger.debug(
            f"\tAdding transaction {(self.sort_code, self.account_number, self.transaction_type, self.amount, self.description, self.pay_date, self.tenant_ref)}"
        )
